...
[g] Generate summary data
[Q] Quit
"""
# Write your application logic here
if __name__ == "__main__":
    manager = WarehouseManager()
    warehouse = Warehouse("Beurs")
    logger = WarehouseLogger(None)
    while True:
...
[T] Get total fee for all packages currently stored
[D] Generate CSV with summary details per date
[B] Generate CSV with the most profitable packages
                  """)
            choice = input("press the corresponding button:\n>").lower()
            if choice == 'f':
                ...
                all_packages = warehouse.manager.get_all_packages_by_delivery_date()
                total_fee = sum(
                    pkg.get_delivery_fee()
                    
                    for pkg_tuple in all_packages.values()
                    for pkg in pkg_tuple
                )
                print(f"Collected fees: {total_fee:.2f} EUR")
            elif choice == 'd':
                all_packages = manager.get_all_packages_by_delivery_date()
                logger.generate_summary_per_date_file(all_packages)
                print("CSV generated.")
            elif choice == 'b':
                amount_packages = input("How many packages do you want to log?\n>")

                try:
                    num_packages = int(amount_packages)
                except ValueError:
                ...
                if logger.generate_most_profitable_packages_file(packages, num_packages):
                    print("CSV generated.")
                else:
                    print("Could not generate CSV.")